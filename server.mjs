...
// -------------------------- OpenAI Call Helper -----------------------------
async function chatAnalyze({ category, brand, dataUrl, description }) {
  const system = buildSystemPrompt(category, brand);

  const hintLine = (description && description.trim().length > 0)
  ? `
Stylist description/hint: "${description.trim()}"`
  : '';
const messages = [
...
tmpPath = req.file.path;
    ...
    let out = await chatAnalyze({ category, brand, dataUrl, description });
